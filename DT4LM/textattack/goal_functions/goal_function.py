...
class GoalFunction(ReprMixin, ABC):
    ...
    def __init__(
        ...
        self.model2 = model_2_wrapper

        if model_2_wrapper:
            validators.validate_model_goal_function_compatibility(
                self.__class__, model_2_wrapper.model.__class__
            )
        ...
        if self.use_cache:
            ...
            self._call_model_2_cache = None

    def clear_cache(self):
        if self.use_cache:
            self._call_model_cache.clear()
            self._call_model_2_cache.clear()
    ...
    def get_output(self, attacked_text):
        ...
        model."""
        # only used in attack_interactive: interactive mode (ignore)
        return self._get_displayed_output(self._call_model([attacked_text])[0]) + "_" + self._get_displayed_output(self._call_model_2([attacked_text])[0])
    ...
    def get_results(self, attacked_text_list, check_skip=False):
        ...
        else: # there exists a second model, conducting model pair attack
            model_2_outputs = self._call_model_2(attacked_text_list)
            for attacked_text, raw_output, raw_output_2 in zip(attacked_text_list, model_outputs, model_2_outputs):
                displayed_output = self._get_displayed_output(raw_output)
                goal_status = self._get_goal_status_2(
                    raw_output, raw_output_2, attacked_text, check_skip=check_skip
                )
    ...
    def _get_goal_status_2(self, model_output, model_2_output, attacked_text, check_skip=False):
        should_skip = check_skip and self._should_skip_2(model_output, model_2_output, attacked_text)
        if should_skip:
            return GoalFunctionResultStatus.SKIPPED
        if self.maximizable:
            return GoalFunctionResultStatus.MAXIMIZING
        if self._is_goal_complete(model_output, model_2_output, attacked_text):
            return GoalFunctionResultStatus.SUCCEEDED
        return GoalFunctionResultStatus.SEARCHING

    @abstractmethod
    def _is_goal_complete(self, *args, **kwargs):
        # allow different implementations, is_goal_complete function can take in both 2 arguments (model_output & attacked_text) or 3 arguments (model_output, model_2_output & attacked_text)
    ...
    def _should_skip_2(self, model_output, model_2_output, attacked_text):
        # to modify!!!
        return self._is_goal_complete(model_output, model_2_output, attacked_text)
    ...
    def _call_model_2_uncached(self, attacked_text_list):
        """Queries model and returns outputs for a list of AttackedText
        objects."""
        if not len(attacked_text_list):
            return []
    ...
    def _call_model_2(self, attacked_text_list):
        ...
        """
        if not self.use_cache:
            return self._call_model_2_uncached(attacked_text_list)
        else:
            uncached_list = []
            for text in attacked_text_list:
                if text in self._call_model_2_cache:
                    # Re-write value in cache. This moves the key to the top of the
                    # LRU cache and prevents the unlikely event that the text
                    # is overwritten when we store the inputs from `uncached_list`.
                    self._call_model_2_cache[text] = self._call_model_2_cache[text]
                else:
                    uncached_list.append(text)
            uncached_list = [
                text
                for text in attacked_text_list
                if text not in self._call_model_2_cache
            ]
            outputs = self._call_model_2_uncached(uncached_list)
            for text, output in zip(uncached_list, outputs):
                self._call_model_2_cache[text] = output
            all_outputs = [self._call_model_2_cache[text] for text in attacked_text_list]
            return all_outputs
    ...
    def __getstate__(self):
        ...
        if self.use_cache:
            ...
            state["_call_model_2_cache"] = self._call_model_2_cache.get_size()
        return state
    ...
    def __setstate__(self, state):
        ...
        if self.use_cache:
            ...
            self._call_model_2_cache = lru.LRU(state["_call_model_2_cache"])
