...
class Trainer:
    ...
    def training_step(self, model, tokenizer, batch):
        ...
        if isinstance(model, transformers.PreTrainedModel) or (
            ...
            # print("Examine if tokenizer has pad token: ", tokenizer.pad_token)
            input_ids.to(textattack.shared.utils.device)
            # print("input_ids: ", input_ids)
            logits = model(**input_ids)[0]
            # print("logits (prediction probabilities): ", logits)
        else:
        ...
        # print("ground truth labels: ", _targets)
    ...
    def train(self):
        ...
        # print("length of original tokenizer: ", len(tokenizer))
        if tokenizer.pad_token is None:
            tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        model = self.model_wrapper.model
        # print("length of (updated) tokenizer: ", len(tokenizer))
