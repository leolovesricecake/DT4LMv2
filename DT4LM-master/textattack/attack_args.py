...
ATTACK_RECIPE_NAMES = {
    ...
    "pair": "textattack.attack_recipes.PAIR2024",
    "kuleshov_var": "textattack.attack_recipes.Kuleshov2017Var",
}
...
GOAL_FUNCTION_CLASS_NAMES = {
    ...
    "differential-classification": "textattack.goal_functions.classification.DifferentialClassification",
...
class _CommandLineAttackArgs:
    ...
    def _add_parser_args(cls, parser):
        ...
        parser.add_argument(
            "--filter-test-by-labels",
            nargs="+", # can accept one or more values
            type=int, # each value provided should be an int
            required=False,
            default=None,
            help="List of labels to keep in the test dataset and discard all others.",
        )
        parser.add_argument(
            "--base-recipe",
            type=str,
            required=False,
            default=default_obj.base_recipe,
        ...
        #     "--do-not-push",
        #     action="store_true",
        #     default=default_obj.do_not_push,
    ...
    def _create_attack_from_args(cls, args, model_wrapper):
        ...
        if args.attack_recipe:
            # print("HERE: ", args.attack_recipe)
            if args.attack_recipe == "pair":
                # if the attack is model pair attack, we need to keep the argument information to load the base attack recipe and second model
                recipe = eval(
                    f"{ATTACK_RECIPE_NAMES[args.attack_recipe]}.build(model_wrapper, args)"
                )
            elif ARGS_SPLIT_TOKEN in args.attack_recipe:
